...
description: >
  ...
  for tini-agent, without leaking API keys or leaving branches behind. Use when
  setting up to test a PR, and ALWAYS after a PR is merged, closed, or parked.
---

# Throwaway worktrees for PR testing

Testing a contributor's PR must never disturb the live dashboard on 9000, and it
...
git -C $REPO worktree add -q ~/Developer/tini-prs/pr$N pr-$N
ln -sfn $REPO/.tini ~/Developer/tini-prs/pr$N/.tini   # real memory/traces/calendar
```
...
```bash
ln -sfn $REPO/.env ~/Developer/tini-prs/pr$N/.env
```
...
```bash
[ -L ~/Developer/tini-prs/pr$N/.env ] && echo "symlink (safe)" || echo "REAL FILE — copy of all keys"
```
...
what the live demo shows (e.g. `TINI_EPISODIC_STORE`) on their old value — Sean
films against the local data.

## Running its dashboard

Always a second port, so 9000 stays untouched:

```bash
cd ~/Developer/tini-prs/pr$N
TINI_DASHBOARD_PORT=7778 $REPO/.venv/bin/python -m tini.ops.dashboard
```
...
for d in ~/Developer/tini-prs/*/; do git -C $REPO worktree remove --force "$d"; done
git -C $REPO worktree prune
git -C $REPO branch --list 'pr-*' | tr -d ' ' | xargs -r -n1 git -C $REPO branch -D
# 4. verify nothing is left holding keys
find ~/Developer/tini-prs -name ".env" 2>/dev/null
```
...
- [ ] `find ~/Developer/tini-prs -name ".env"` prints nothing
