# Model comparison — the Tini benchmark battery

How Tini compares models, what each test measures, and how to read the result.
...
Multi-step orchestration over Tini's flagship tools (`create_event`,
...
Tini is the orchestrator; **pi** is the coding contractor — but for a *coding*
...
**moonshotai/kimi**, xai/grok, zai/glm) — the runner maps Tini's provider id to
...
[`tini/ops/coding_eval.py`](../tini/ops/coding_eval.py).
...
(`./tini_workspace/<date>/<time>-<model>-<slug>/`, git-ignored) with a
...
`TINI_WORKSPACE` (root), `TINI_DELEGATE_AUTORUN=0` (disable), `TINI_AUTORUN_TIMEOUT`.
See [`tini/tools/workspace.py`](../tini/tools/workspace.py).
...
Yes, Tini already does Hermes / Claude-Code-style sub-agent spawning. It lives in
[`tini/tools/experimental.py`](../tini/tools/experimental.py) as `delegate_task`
and is **off by default** — set `TINI_EXPERIMENTAL=1` to register it.
...
- **What it is:** the "Sub-Agents" box on the architecture whiteboard, wired for
  ...
  (`pi -p "task"`).
- **The division of labor is the teaching point:** Tini is the orchestrator
  (memory, working-memory assembly, the human's context, the release gate); pi
  is the specialist contractor (read / bash / edit / write). Tini hires; pi
  codes; Tini's gate inspects the work.
...
[`tini/ops/judge.py`](../tini/ops/judge.py). The referee is **switchable from the
...
(grading is pure text). Any provider works — Tini's OpenAI-compat client gives
...
#   → also writes a timestamped .md + .json to .tini/shootout/

# Live arena — race every pinned model on one prompt, watch it stream
make dashboard            # localhost:9000 → Arena tab
...
**What Tini already has:** the case format (`dataset.jsonl`), deterministic
...
- ~~Completion column wired into the live arena~~ — **done**: a race on a known
  ...
  [`tini/ops/scoring.py`](../tini/ops/scoring.py) shared with `shootout.py`.
...
- ~~Quality column (K3-as-judge) in the arena~~ — **done**: the "grade with K3"
  toggle judges each reply 0-10 ([`tini/ops/judge.py`](../tini/ops/judge.py));
...
### Setup
1. `make dashboard` → open `localhost:9000` → **Compare** tab.
...
markdown + json report to `.tini/shootout/` anyone can reproduce with their keys.
