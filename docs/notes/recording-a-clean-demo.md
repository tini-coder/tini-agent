...
A working note, not reference. Moved out of the README.

## Recording a clean demo

```bash
python scripts/demo_seed.py --yes      # resets .tini to a tidy, curated state (--yes required)
```

It backs up your current `.tini` first, then seeds a few clean facts, one episode, and one
