...
# The pi × tini video — shooting rundown
...
```
pi --mode json -p --no-session -nt "Reply with exactly: tini tini"
```
...
```
claude -p "Reply with exactly: tini tini"     # CLI: answers and dies
claude "Reply with exactly: tini tini"        # TUI: banner, state, waits
```
...
carries the model; pi shows you the model. That's why tini's arena delegates
coding to pi."*

## Segment D — tini hires pi (the demo)

```
TINI_EXPERIMENTAL=1 make run
```
```
...
Receipts: `python -m tini.ops.show_trace` for the delegation in the trace.
