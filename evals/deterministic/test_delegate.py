...
def _tmp_workspace(tmp_path, monkeypatch):
    """Never let a delegate test write into the repo's ./tini_workspace."""
    monkeypatch.setenv("TINI_WORKSPACE", str(tmp_path / "ws"))
...
def test_delegate_task_invokes_pi_print_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TINI_WORKSPACE", str(tmp_path / "ws"))   # keep it out of the repo
...
def test_delegate_timeout_is_honest(tmp_path, monkeypatch):
    ...
    assert "7s" in out and "TINI_DELEGATE_TIMEOUT" in out
...
def test_delegate_kills_a_silent_pi_at_the_deadline(tmp_path, monkeypatch):
    ...
    assert "2s" in out and "TINI_DELEGATE_TIMEOUT" in out
...
def test_experimental_flag_gates_registration(tmp_path, monkeypatch):
    ...
    app_off = make_tini(tmp_path / "off", client=ScriptedClient([]))
