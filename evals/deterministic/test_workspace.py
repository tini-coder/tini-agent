"""DETERMINISTIC EVAL — the delegated-coding workspace (tini.tools.workspace).
...
from tini.tools import workspace as ws


def test_run_folder_is_dated_and_named(tmp_path, monkeypatch):
    monkeypatch.setenv("TINI_WORKSPACE", str(tmp_path))
...
def test_autorun_disabled_by_env(tmp_path, monkeypatch):
    monkeypatch.setenv("TINI_WORKSPACE", str(tmp_path))
    monkeypatch.setenv("TINI_DELEGATE_AUTORUN", "0")
