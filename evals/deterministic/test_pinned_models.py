...
The shortlist lives in .tini/models.json as {"pinned": ["provider:model", ...]};
...
from tini.ops import settings_api as d
...
def home(tmp_path, monkeypatch):
    ...
    # so TINI_PROVIDER must also be tracked to prevent leaking into later
...
def test_switching_provider_adopts_its_pinned_default(home, monkeypatch):
    ...
    monkeypatch.setenv("TINI_PROVIDER", "gemini")
    monkeypatch.setenv("TINI_MODEL", "gemini-3.5-flash")
    result = integrations.apply_provider("kimi")
    assert result.ok
    assert os.getenv("TINI_PROVIDER") == "kimi"
    assert os.getenv("TINI_MODEL") == "kimi-k3"  # not gemini's model
