...
from tini.ops.dashboard import collect, events_since
from tini.ops.tracing import TraceEncodingError, Tracer
...
def test_dashboard_reads_utf8_without_platform_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TINI_HOME", str(home))
    original_open = Path.open
...
def test_dashboard_ops_view_surfaces_trace_encoding_errors():
    views = (
        Path(__file__).resolve().parents[2] / "tini" / "ops" / "static" / "js" / "views.js"
