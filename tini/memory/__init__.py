...
from tini.memory import consolidation, retrieval_gate
...
def bundled_skill_dirs() -> list[Path]:
    """Where the skills that SHIP with Tini live — and why there are two answers.
    ...
    the wheel only packages the `tini/` directory, so a `pip install tini-agent`
    ...
    `tini/skills`, so an installed Tini finds it next to the code.
...
class Memory:
    ...
    def export_markdown(self) -> None:
        ...
        whiteboard's `~/.tini/MEMORY.md` box is literally real, and "your memory
        ...
        lines = [
            "# Tini memory",
            "",
            ("_A human-readable mirror of what Tini remembers. The source of truth is "
