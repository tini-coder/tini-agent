"""Skill installer — try anyone's skill in one command.
    ...
    python -m tini skill install https://github.com/<user>/<repo>/blob/main/skills/foo/SKILL.md
    python -m tini skill install https://gist.github.com/<user>/<id>

Downloads the SKILL.md, validates the frontmatter (same check CI runs on
community PRs), and drops it in TINI_HOME/skills/<name>/ where the loader
...
from tini.memory.procedural.loader import _parse
...
def install(url: str) -> None:
    ...
    print("It loads next time Tini starts. Read it first — skills are instructions.")
