// tini dashboard — render/refresh loop, resizers/chrome, voice, bootstrap (LOADS LAST).
...
const TITLES = {chat:"Chat & watch", ops:"LLM Ops",
                ...
                database:"Database — everything Tini stores (state.db)"};
function render(){
  if (!D) return;
...
const micHint = (msg) => { const i = document.getElementById("dmsg");
  if (i){ i.placeholder = msg; setTimeout(()=>{ i.placeholder = "Message Tini…"; }, 8000); } };

async function toggleMic(){
  const btn = document.getElementById("mic");
  if (micOn){ await stopMic(); return; }
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){
    micHint("voice needs a normal browser tab at localhost:9000 — not the IDE preview pane");
    return;
  }
  try {
