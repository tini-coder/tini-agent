// tini dashboard — formatters + chat card renderers + chatlog + streaming + send.
...
const streamingCard = m => `<div class="card">
  ...
  ${m.stream
     ...
     : `<div class="meta" style="margin:0">thinking&hellip;${m.started?` ${Math.round((Date.now()-m.started)/1000)}s`:""}${
         ...
         ? `<br>still waiting: slow models (free tiers especially) can queue for a while; this errors out at the TINI_LLM_TIMEOUT limit instead of hanging forever`
         : ""}</div>`}
</div>`;
...
function renderChatLog(){
  if (!CHAT.length)
    return `<div class="empty" style="padding:6px 2px">Message Tini here from any tab. Open Overview to watch it flow through the harness, or the Gateway tab to see every channel's messages together.</div>`;
...
async function sendChat(fromInput){
  ...
  const pending = {role:"tini", pending:true, stream:"", started: Date.now()};
  CHAT.push(pending);
  syncChatLogs();
