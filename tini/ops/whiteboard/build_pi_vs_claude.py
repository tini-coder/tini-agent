...
aligned rows each, then the tini punchline. Facts are sourced on the board
...
Run:  python -m tini.ops.whiteboard.build_pi_vs_claude
"""
...
ROWS = [
    ...
    ("SUB-AGENTS",
     ...
     "refused — spawn pi yourself: tmux,\nor tini's delegate_task (that's us)",
     "plain"),
    ("MCP",
...
def build() -> list:
    ...
    e += S.labeled_box(80, band, 2410, 96,
                       "tini uses BOTH: Claude Code writes tini's code - pi is tini's delegate_task sub-agent,\n"
