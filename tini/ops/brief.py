"""`python -m tini brief` — a morning briefing that runs through the normal
...
def main() -> None:
    console = Console()
    tini = Tini()
    if not tini.settings.apple_tools:
        console.print("[dim]Tip: set TINI_APPLE_TOOLS=1 to brief from your real Calendar and Mail.[/dim]")
    result = tini.respond(PROMPT, source="brief")
    console.print(result.reply)
    out = tini.settings.home / "outbox" / f"brief-{date.today().isoformat()}.txt"
