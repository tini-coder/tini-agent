"""Reset .tini to a clean, curated state for a demo / recording.
...
What it does (your old state is backed up first, never just deleted):
  1. moves the current .tini aside to .tini.bak-<timestamp>
...
def main(reset_spend: bool = False) -> None:
    ...
    print("  Run `tini dashboard` and start filming.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reset .tini to a clean demo state.")
    parser.add_argument("--yes", "-y", action="store_true",
                        help="required confirmation: yes, wipe .tini (it is backed up first)")
    ...
    if not args.yes:
        ...
        print("REFUSING to run: demo_seed clears .tini (memory, calendar, chat, traces"
