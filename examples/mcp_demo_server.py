"""A tiny, self-contained MCP server — the demo connector for tini-agent.
...
so the connector story runs with zero extra installs:
    ...
    pip install -e '.[mcp]'
    cp examples/mcp.demo.json .tini/mcp.json
...
that's the whole point: connectors plug in without changing Tini's code.
"""
...
if __name__ == "__main__":
    mcp.run()  # stdio transport — how Tini's MCPBridge talks to it
